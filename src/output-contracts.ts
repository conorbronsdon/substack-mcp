...
export const objectOutputSchemas: Record<string, z.AnyZodObject> = {
  ...
  search_subscribers: z.object({ total_matching: count, returned: count.max(50), offset: count, limit: count.min(1).max(50), has_more: z.boolean(), next_offset: count.nullable(),
    ...
    subscribers: z.array(subscriber.extend({ activity_rating: z.number().finite().optional(), created_at: text.optional(), revenue: z.number().finite().optional(),
      is_comp: z.boolean().optional(), is_founding: z.boolean().optional(), is_gift: z.boolean().optional(), is_free_trial: z.boolean().optional() })).max(50) }),
