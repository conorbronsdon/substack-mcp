...
export const objectOutputSchemas: Record<string, z.AnyZodObject> = {
  ...
  get_post_analytics: z.object({ found: z.boolean(), source: z.enum(["post_detail", "published_feed_scan"]).optional(), detail_fallback_reason: z.enum(["not_found", "malformed", "id_mismatch", "forbidden"]).optional(), post_id: id.optional(), note: text.optional(), search_result: z.enum(["archive_exhausted", "scan_bound_reached", "feed_incomplete"]).optional(), scanned: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).optional(), /* separate instance: sharing count would make existing fields $ref this one */ feed_capped: z.boolean().nullable().optional(), stats_available: z.boolean().optional(), id: id.optional(), title: maybeText, post_date: maybeText,
