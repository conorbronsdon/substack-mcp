...
| [PulseMCP](https://www.pulsemcp.com/servers/conorbronsdon-substack) | Exact repository and registry name `io.github.conorbronsdon/substack-mcp`; links the registry `server.json` | Not displayed | Summary: long-form posts are draft-only; Notes publish immediately. Displayed GitHub star count is out of date | Current identity; confirmed listing | Registry ingestion from `server.json`; no correction submitted |
...
| [MCP Market](https://mcpmarket.com/server/substack-4) | Not verified | Unavailable | Unavailable | Unverified: two fetches returned HTTP 429 from a security checkpoint | Recheck in a browser; verify owner, package and version before any correction |
