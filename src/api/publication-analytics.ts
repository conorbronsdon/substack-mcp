...
const reason = z.enum(["http_401", "http_403", "http_404", "http_429", "http_5xx", "http_other", "malformed", "timeout", "response_too_large", "redirect_rejected", "unexpected_html"]);
...
const summaryRaw = z.object(Object.fromEntries(summaryFields.map(k => [k, finite.nullable().optional()])) as Record<(typeof summaryFields)[number], z.ZodOptional<z.ZodNullable<typeof finite>>>).extend({ pledgeCurrency: z.string().max(32).nullish(), isBestseller: z.boolean().nullish() });
...
const group = z.object({ status: z.literal("available"), source: z.string(), captured_at: z.string(), metrics: z.record(metric), is_bestseller: z.boolean().nullable().optional() });
...
function reasonFor(error: unknown): z.infer<typeof reason> {
  ...
  if (error instanceof ResponseError) {
    ...
    return error.code === "malformed_json" ? "malformed" : error.code;
  }
...
async function readGroup<T, R>(path: string, schema: z.ZodType<T>, project: (raw: T) => R, read: Read): Promise<{ value: R; error: null } | { value: z.infer<typeof unavailable>; error: SubstackAPIError }> {
  try {
    ...
    const failureReason = reasonFor(error);
    return { value: { status: "unavailable", reason: failureReason }, error: error as SubstackAPIError };
  }
}
...
export const growthSourcesInput = z.object({ from_date: day, to_date: day, limit: z.number().int().min(1).max(50).default(20), include_timeseries: z.boolean().default(false), include_events: z.boolean().default(false) }).strict().superRefine((v, ctx) => {
  ...
  const tomorrow = Math.floor(Date.now() / 86_400_000) * 86_400_000 + 86_400_000;
  if (from > to || to - from > 365 * 86_400_000 || to > tomorrow) ctx.addIssue({ code: "custom", message: "Date range must be ordered, at most 366 inclusive days, and end by tomorrow UTC." });
});
...
// Note events carry the Note text as `title` (785 characters observed live) and may omit
// `slug`/`url` (observed live; projected as null), so the raw
// schema accepts long titles and the projection caps them with an explicit flag.
const EVENT_TITLE_MAX = 300;
const event = z.object({ id: z.number().int().positive(), date: short, title: z.string().max(100_000), slug: short.nullish(), type: short, url: short.nullish() });
const projectedEvent = z.object({ id: z.number().int().positive(), date: short, title: z.string().max(EVENT_TITLE_MAX), title_truncated: z.boolean(), slug: short.nullable(), type: short, url: short.nullable() });
...
export async function getGrowthSources(input: z.input<typeof growthSourcesInput>, read: Read) {
  ...
  const events = v.include_events ? (await readGroup(`/api/v1/publication/stats/growth/events?from_date=${v.from_date}&to_date=${v.to_date}`, eventsRaw, raw => ({ status: "available", items: raw.pubEvents.map(e => ({ ...e, slug: e.slug ?? null, url: e.url ?? null, title: e.title.slice(0, EVENT_TITLE_MAX), title_truncated: e.title.length > EVENT_TITLE_MAX })) }), read)).value : undefined;
  return { from_date: v.from_date, to_date: v.to_date, sources, totals: root.data.totals, returned: sources.length, total_sources: root.data.sourceMetrics.length, has_more: sources.length < root.data.sourceMetrics.length, truncated, ...(events ? { events } : {}) };
}
