  ...
  it("classifies any public schema drift as potentially breaking until reviewed", async () => {
    for (const multi of [false, true]) {
      ...
      try {
        ...
        expect(tools.filter(t => t.outputSchema)).toHaveLength(24);
  ...
  it("projects representative client results into every legacy object/array read and write contract", async () => {
    ...
    try {
      ...
      const cases = [
        ...
        ["get_post_analytics", { post_id: 42 }, "findExactPostAnalytics", { post: { ...post, stats: { views: 5 } }, outcome: "found", scanned: 1, feed_capped: false, source: "post_detail" }],
