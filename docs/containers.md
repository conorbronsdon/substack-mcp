...
```sh
docker run -i --rm --env-file ./substack.env ghcr.io/conorbronsdon/substack-mcp:1.1.1
```
...
```sh
docker run --rm --env-file ./substack.env --entrypoint node ghcr.io/conorbronsdon/substack-mcp:1.1.1 dist/index.js status --json
```
...
```sh
docker run --rm --env-file ./substack.env -p 127.0.0.1:8080:8080 -e MCP_TRANSPORT=http -e MCP_HTTP_ALLOWED_HOSTS=127.0.0.1:8080,localhost:8080 ghcr.io/conorbronsdon/substack-mcp:1.1.1
```
