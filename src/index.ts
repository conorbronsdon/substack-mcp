...
async function run() {
  ...
  if (args.length === 1 && ["--help", "-h"].includes(args[0])) {
    console.log("Usage: substack-mcp [serve | login [options] | profiles list/migrate [options] | status [--json] | doctor [--json] [--check-auth] | export <draft-id> [options] | drafts list/get/create/preflight/export/plan/apply [options] | posts search <query> | analytics post <id> | analytics rank [options] | subscribers count/get]\nWith no command, starts the MCP server. doctor checks configuration without network access; --check-auth adds a bounded read per publication. export saves Markdown and original JSON without changing Substack; run export --help. For reviewed updates, run drafts --help. Read commands: drafts list/get/preflight, posts search, analytics post/rank, subscribers count/get. drafts create writes one private, unpublished draft from a Markdown file. Add --publication when needed. status is offline. Login: substack-mcp login (substack-mcp-login remains an alias). Named sessions: profiles --help.");
    return;
  }
